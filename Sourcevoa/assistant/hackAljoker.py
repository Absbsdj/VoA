...
'''
mm = '''
قم بلأنضمام الى قناة  VoA @X_YI1I
'''
...
keyboard = [
  ...
  [
    Button.url("سورس VoA🤡", "https://t.me/X_YI1I")
    ]
]
  ...
  if event.sender_id == bot.uid:
      async with bot.conversation(event.chat_id) as x:
        keyboard = [
          ...
          [
            Button.url("المطور", "https://t.me/X_Y1I")
            ]
        ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nشكرا لأستخدامك سورس  VoA", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i == True:
          ...
          await event.reply(f"حدث خطأ قم بتوجيه الرسالة للمطور @X_Y1I\n{i}")
